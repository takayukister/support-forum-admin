...
## サポートフォーラムを支えるシステムについて

WordPress.org のサポートフォーラムはそれ自体が WordPress を基盤として構築されたシステムであり、bbPress をはじめとしたいくつかのプラグインがその機能を支えています。
...
* 英語版にのみ各プラグイン、テーマ専門のフォーラムカテゴリーが用意されています。専門カテゴリーに作成されたトピックに対しては対象のプラグイン、テーマの開発者に対して特別な権限 (トピックステータスの変更) が追加付与されます。
...
サポートフォーラムには bbPress の標準権限グループに基づいた以下の権限グループがあります:
...
フォーラムには毎日多数の投稿があります。それらを広く細かくチェックすることが理想ですが、限られたリソースで全てを見回ることは不可能ですので、以下の重要項目を優先的にチェックすることをお勧めします。
...
これらの作業もモデレーターに期待されるものですが、時間の制約もありなかなかすべての投稿をチェックして回るのは難しいかと思います。もし時間に余裕があれば、できる範囲でやってみてください。
